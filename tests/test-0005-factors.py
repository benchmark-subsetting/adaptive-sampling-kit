...
class DriverTests(CommandLineT):
    def test_ranges(self):
        ...
        assert r.stderr.find("min") != -1, "error message should mention problem with min"
    ...
    def test_invalid_types(self):
        ...
        r = self.run("ask test.conf", expect_error=True)
        assert r.returncode != 0, "Return code should be != 0"
        assert r.stderr.find("type") != -1, "error message should mention problem with type"
    ...
    def test_bad_categorical(self):
        ...
        r = self.run("ask test.conf", expect_error=True)
        assert r.returncode != 0, "Return code should be != 0"
        assert r.stderr.find("Values") != -1, "error message should mention problem with values"
    ...
    def test_bad_integer(self):
        ...
        r = self.run("ask test.conf", expect_error=True)
        assert r.returncode != 0, "Return code should be != 0"
        assert r.stderr.find("range") != -1, "error message should mention problem with range"
    ...
    def test_repeated_name(self):
        ...
        r = self.run("ask test.conf", expect_error=True)
        assert r.returncode != 0, "Return code should be != 0"
        assert r.stderr.find("name") != -1, "error message should mention problem with name"
